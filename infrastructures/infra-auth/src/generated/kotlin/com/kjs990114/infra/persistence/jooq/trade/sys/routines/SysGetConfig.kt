 ...
 *    * The convention is to name the user variables @sys.&lt;name of
 * variable&gt;. It is &lt;name of variable&gt; that
 *      is stored in the sys_config table and is what is expected as the
 * argument to sys_get_config().
 ...
 *   The default value to return if the variable does not exist in
 * sys.sys_config.
 * 
 * Returns
 * -----------
 ...
 * 
 * -- Check whether the option is already set, if not assign - IFNULL(...) one
 * liner example.
 ...
 *     SET @sys.statement_truncate_len =
 * sys.sys_get_config('statement_truncate_len', 64);
 * END IF;
...
open class SysGetConfig : AbstractRoutine<String>("sys_get_config", Sys.SYS, SQLDataType.VARCHAR(128)) {
    ...
    init {
        returnParameter = SysGetConfig.RETURN_VALUE
        addInParameter(SysGetConfig.IN_VARIABLE_NAME)
        addInParameter(SysGetConfig.IN_DEFAULT_VALUE)
    }

    /**
     * Set the <code>in_variable_name</code> parameter IN value to the routine
     */
    fun setInVariableName(value: String?): Unit = setValue(SysGetConfig.IN_VARIABLE_NAME, value)

    /**
     * Set the <code>in_variable_name</code> parameter to the function to be
     * used with a {@link org.jooq.Select} statement
     */
    fun setInVariableName(field: Field<String?>): SysGetConfig {
        setField(SysGetConfig.IN_VARIABLE_NAME, field)
        return this
    }
     ...
     */
    fun setInDefaultValue(value: String?): Unit = setValue(SysGetConfig.IN_DEFAULT_VALUE, value)

    /**
     * Set the <code>in_default_value</code> parameter to the function to be
     * used with a {@link org.jooq.Select} statement
     */
    fun setInDefaultValue(field: Field<String?>): SysGetConfig {
        setField(SysGetConfig.IN_DEFAULT_VALUE, field)
        return this
    }
}
